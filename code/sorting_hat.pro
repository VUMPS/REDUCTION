...
if keyword_set(getthid) then begin
	...
	if keyword_set(thar_soln) then begin
		thidfile =  thid_path2+thar_soln+'.thid' 
	endif else begin 	
		findthid, date=date, redpar, thidfile
	...
	fnm = thid_path2+date+'/'+redpre+image_prefix+thar[i]
	fsuf = '.thid'
...
if keyword_set(iod2fits) then begin
	x1=where(slit eq redpar.resolutionarr[resolutionidx] $
	and objnm ne 'junk' and objnm ne 'dark' $
	and objnm ne 'focus' and objnm ne 'bias',n_found)

	tharindx=where((objnm eq 'thar') and (slit eq redpar.resolutionarr[resolutionidx]),  num_thar)
	...
	if keyword_set(thar_soln) then begin
		restore, thid_path2+date+'/'+thar_soln+'.thid' ; thid structure
		mkwave, w, thid.wvc
		...
		thidfiles = thid_path2+date+'/'+redpre+image_prefix+obnm[tharindx]+'.thid' ; string array of wavelength solution file names  
		;stop
	...
	spec[1,*,*]=rotate(sp,2)
	outfile=redpre+image_prefix+obnm[i]+'.fits'
	;*******************************************************
